...
export default function PomodoroPage() {
  ...
  // Active settings — passed to usePomodoro hook
  ...
  // Pending settings — edited inside the modal, only applied on "Apply"
  ...
  // Open settings modal — sync pending from active
  const openSettings = () => {
    setPendingWork(workMinutes);
    setPendingBreak(breakMinutes);
    setPendingLongBreak(longBreakMinutes);
    setPendingSessions(sessionsBeforeLongBreak);
    setPendingAutoBreak(autoStartBreak);
    setPendingAutoWork(autoStartWork);
    setShowSettings(true);
  };

  // Apply settings — copy pending → active and persist
  const applySettings = () => {
    setWorkMinutes(pendingWork);
    setBreakMinutes(pendingBreak);
    setLongBreakMinutes(pendingLongBreak);
    setSessionsBeforeLongBreak(pendingSessions);
    setAutoStartBreak(pendingAutoBreak);
    setAutoStartWork(pendingAutoWork);
    saveSettings({
      workMinutes: pendingWork,
      breakMinutes: pendingBreak,
      longBreakMinutes: pendingLongBreak,
      sessionsBeforeLongBreak: pendingSessions,
      autoStartBreak: pendingAutoBreak,
      autoStartWork: pendingAutoWork,
    });
  ...
  const handleSessionComplete = async (durationMinutes: number, phase: PomodoroPhase) => {
    ...
    if (subjectId) {
      await supabase.rpc("increment_subject_minutes", { sid: subjectId, mins: durationMinutes });
    }

    const { data: profile } = await supabase
      .from("profiles").select("total_study_minutes").eq("id", userId).single();

    await supabase
      .from("profiles")
      .update({ total_study_minutes: (profile?.total_study_minutes ?? 0) + durationMinutes })
      .eq("id", userId);
    ...
    toast({
      title: `Session saved! +25 XP 🎉`,
      description: leveledUp
        ? `You reached Level ${newLevel}! 🚀`
        : `${durationMinutes} min logged. Keep going!`,
    });
  };
  ...
  const { phase, isRunning, formattedTime, progress, sessionCount, start, pause, reset, skipPhase, setPhase } =
    usePomodoro({
      ...
      settings: { workMinutes, breakMinutes, longBreakMinutes, sessionsBeforeLongBreak, autoStartBreak, autoStartWork },
    });
  ...
  const pendingSliders = [
    { label: "Focus Duration", value: pendingWork, min: 5, max: 120, step: 5, setter: setPendingWork, unit: "min" },
    { label: "Short Break", value: pendingBreak, min: 1, max: 30, step: 1, setter: setPendingBreak, unit: "min" },
    { label: "Long Break", value: pendingLongBreak, min: 5, max: 60, step: 5, setter: setPendingLongBreak, unit: "min" },
    { label: "Sessions → Long Break", value: pendingSessions, min: 1, max: 8, step: 1, setter: setPendingSessions, unit: "sessions" },
  ];
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-display font-black text-2xl">Pomodoro Timer</h1>
          <p className="text-muted-foreground text-sm mt-1">Deep work in focused intervals</p>
        </div>
          ...
          <Card className="overflow-hidden">
              ...
              {(["work", "break", "longBreak"] as PomodoroPhase[]).map((p) => (
                <button
                  ...
                  className={cn(
                    ...
                    phase === p ? "border-b-2 text-foreground" : "text-muted-foreground hover:text-foreground"
                  )}
            ...
            <CardContent className="p-8 flex flex-col items-center gap-8">
              ...
              <div className="flex items-center gap-2">
                ...
                <Badge
                  style={{ background: `${phaseColor}20`, color: phaseColor, borderColor: `${phaseColor}40` }}
                  className="border"
                >
                  {PHASE_LABELS[phase]}
                </Badge>
              ...
              <div className="relative">
                <svg width="220" height="220" viewBox="0 0 220 220">
                  {isRunning && (
                    <circle
                      cx="110" cy="110" r={R + 8}
                      fill="none" stroke={phaseColor} strokeWidth="1" opacity="0.2"
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  ...
                  <span className="text-xs text-muted-foreground mt-1">Session #{sessionCount + 1}</span>
                </div>
              </div>
              ...
              <div className="flex items-center gap-4">
                ...
                <Button
                  ...
                  style={{ background: phaseColor, color: "#0D0D18", boxShadow: `0 0 30px ${phaseColor}50` }}
                >
                  {isRunning ? <Pause className="w-8 h-8" /> : <Play className="w-8 h-8 translate-x-0.5" />}
                </Button>
                <Button variant="ghost" size="icon" onClick={skipPhase} title="Skip (logs actual time studied)">
        ...
        <div className="space-y-4">
          <Card>
            <CardContent className="p-5 space-y-4">
              <p className="font-display font-semibold text-sm">Today&apos;s Progress</p>
              <div className="grid grid-cols-2 gap-3">
                {[
                  {
                    label: "Sessions",
                    value: sessionHistory.filter((s) => new Date(s.started_at).toDateString() === new Date().toDateString()).length,
                    icon: "⚡",
                  },
                  { label: "Focused", value: formatMinutes(todayMinutes), icon: "🎯" },
                  {
                    label: "XP Earned",
                    value: `+${sessionHistory.filter((s) => new Date(s.started_at).toDateString() === new Date().toDateString()).length * 25}`,
                    icon: "💫",
                  },
                  { label: "All-time", value: sessionHistory.length, icon: "📊" },
                ].map((stat) => (
                ...
                sessionHistory.slice(0, 6).map((session) => (
                  <div key={session.id} className="flex items-center gap-3 p-2 rounded-lg hover:bg-muted/50 transition-colors">
                    ...
                    >
                      <CheckCircle2
                        className="w-4 h-4"
                        style={{ color: session.subject?.color ?? "hsl(var(--muted-foreground))" }}
                      />
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-medium truncate">
                        {session.notes || session.subject?.name || "Focus session"}
                      </p>
                    ...
                    <div className="text-right">
                      <p className="text-xs font-bold text-lime">{session.duration_minutes}m</p>
          ...
          <Card className="border-lime/20 bg-lime/5">
            <CardContent className="p-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-lime">Pro Tip</p>
                  <p className="text-xs text-muted-foreground mt-1">
                    Skipping logs only the time you actually studied — every minute counts!
                  </p>
                </div>
              </div>
            </CardContent>
          ...
          >
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-border">
              <div className="flex items-center gap-2">
                ...
                <h2 className="font-display font-bold text-base">Timer Settings</h2>
              </div>
              <button
                onClick={() => setShowSettings(false)}
            ...
            <div className="px-6 py-5 space-y-6">
                  ...
                  <div key={label} className="space-y-3">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="text-lime font-bold">
                        {value} {unit === "sessions" ? (value === 1 ? "session" : "sessions") : "min"}
                      </span>
                    </div>
                    <input
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  { label: "Auto-start breaks", value: pendingAutoBreak, setter: setPendingAutoBreak },
                  { label: "Auto-start focus", value: pendingAutoWork, setter: setPendingAutoWork },
                  ...
                  >
                    <div className={cn(
                      ...
                      value ? "bg-lime" : "bg-muted-foreground/30"
                    )}>
                      <div className={cn(
            ...
            <div className="flex items-center justify-between px-6 py-4 border-t border-border">
              <p className="text-xs text-muted-foreground">Changes take effect after applying</p>
              <div className="flex items-center gap-2">
