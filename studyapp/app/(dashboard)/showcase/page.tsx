...
import {
  ...
  X, Star, ArrowLeft, Maximize2, Minimize2,
} from "lucide-react";
...
const FOLDER_COLORS = [
  "#00b7ff", "#4ECDC4", "#FF6B6B", "#A78BFA",
  "#F59E0B", "#34D399", "#F472B6", "#60A5FA",
];

const LS_FILES = "nahda:showcase:files";
const LS_FOLDERS = "nahda:showcase:folders";
...
function loadFiles(): PDFFile[] {
  try { const r = localStorage.getItem(LS_FILES); return r ? JSON.parse(r) : []; } catch { return []; }
}
function loadFolders(): PDFFolder[] {
  try { const r = localStorage.getItem(LS_FOLDERS); return r ? JSON.parse(r) : []; } catch { return []; }
}
function saveFiles(f: PDFFile[]) {
  try { localStorage.setItem(LS_FILES, JSON.stringify(f)); } catch { }
}
function saveFolders(f: PDFFolder[]) {
    ...
    >
      {items.map((item) => (
        <button key={item.label} onClick={() => { item.onClick(); onClose(); }}
          className={cn("w-full flex items-center gap-3 px-4 py-2.5 text-sm font-medium transition-colors text-left",
...
function PDFViewer({ file, onClose }: { file: PDFFile; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-[#08080F] flex flex-col">
      ...
      <div className="h-14 border-b border-white/10 flex items-center justify-between px-6 shrink-0 bg-[#0e0e1a]/80 backdrop-blur">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setFullscreen((f) => !f)}
            className="w-8 h-8 rounded-xl flex items-center justify-center bg-white/5 hover:bg-[#00b7ff]/20 text-white/40 hover:text-[#00b7ff] transition-colors"
      ...
      <div className="flex-1 overflow-auto flex items-start justify-center p-8 bg-[#06060d]">
        <div style={{ transform: `scale(${zoom / 100})`, transformOrigin: "top center", transition: "transform 0.2s ease" }}>
          <embed
            ...
            style={{ width: fullscreen ? "90vw" : "816px", height: "80vh", display: "block" }}
          />
        </div>
      </div>
      ...
      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-3 px-4 py-2 rounded-full bg-white/5 border border-white/10 backdrop-blur pointer-events-none">
        {[["ESC", "close"], ["+/-", "zoom"], ["F", "fullscreen"]].map(([key, label]) => (
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: "rgba(8,8,15,0.8)" }} onClick={onClose}>
      <div className="w-80 bg-[#0e0e1a] border border-white/10 rounded-2xl p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex gap-2 mt-4 justify-end">
          ...
          <button onClick={() => { onSave(val); onClose(); }} className="px-4 py-2 rounded-xl text-sm font-semibold bg-[#00b7ff] text-black">Save</button>
        </div>
      </div>
    </div>
  );
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: "rgba(8,8,15,0.8)" }} onClick={onClose}>
      <div className="w-80 bg-[#0e0e1a] border border-white/10 rounded-2xl p-6 shadow-2xl space-y-5" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex gap-2 justify-end">
          <button onClick={onClose} className="px-4 py-2 rounded-xl text-sm text-white/50 hover:text-white hover:bg-white/5 transition-colors">Cancel</button>
          <button onClick={() => { onSave(name, color); onClose(); }} className="px-4 py-2 rounded-xl text-sm font-semibold bg-[#00b7ff] text-black">Create</button>
        </div>
      </div>
    </div>
  );
...
export default function ShowcasePage() {
  ...
  const createFolder = (name: string, color: string) =>
    setFolders((prev) => [...prev, { id: crypto.randomUUID(), name, color, createdAt: new Date().toISOString(), parentId: currentFolderId }]);
    ...
    >
      ...
      <div className="flex items-center justify-between mb-6 gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => inputRef.current?.click()}
            className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold bg-[#00b7ff] text-black hover:bg-[#00b7ff]/90 transition-all"
      ...
      <div className="flex items-center gap-3 mb-4 flex-wrap">
          ...
          {breadcrumbs.map((crumb, i) => (
            <span key={crumb.id ?? "root"} className="flex items-center gap-1">
              ...
              <button onClick={() => setCurrentFolderId(crumb.id)}
                className={cn("px-2 py-1 rounded-lg transition-colors font-medium",
                  i === breadcrumbs.length - 1 ? "text-white" : "text-white/40 hover:text-white hover:bg-white/5")}>
        ...
        <select value={sortKey} onChange={(e) => setSortKey(e.target.value as SortKey)}
          className="px-3 py-1.5 rounded-xl border border-white/10 bg-white/5 text-sm text-white/60 focus:outline-none cursor-pointer">
      ...
      {!currentFolderId && !search && starredFiles.length > 0 && (
        <div className="mb-5">
          ...
          <div className="flex gap-3 overflow-x-auto pb-1">
            {starredFiles.map((f) => (
              <button key={f.id} onClick={() => setViewerFile(f)}
      ...
      <div className="flex-1 overflow-auto min-h-0">
        {isEmpty && !search ? (
          <div className="flex flex-col items-center justify-center h-80 rounded-2xl border-2 border-dashed border-white/10 cursor-pointer hover:border-[#00b7ff]/40 hover:bg-[#00b7ff]/5 transition-all group"
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3">
            {/* Folders — single click */}
            {visibleFolders.map((folder) => (
              <div key={folder.id}
                className="group relative rounded-2xl border border-white/8 bg-white/3 hover:bg-white/6 transition-all cursor-pointer p-4 flex flex-col items-center gap-2 hover:border-white/15 select-none"
                onClick={() => setCurrentFolderId(folder.id)}
                onContextMenu={(e) => openCtxMenu(e, "folder", folder.id)}
              >
                <Folder className="w-12 h-12 transition-transform group-hover:scale-105"
              ...
              >
                <div className="h-28 flex items-center justify-center relative overflow-hidden"
                  ...
                  <div className="absolute inset-0 flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity"
                    ...
                    <button onClick={() => setViewerFile(file)}
                      className="w-9 h-9 rounded-xl bg-[#00b7ff] text-black flex items-center justify-center hover:scale-110 transition-transform" title="Open">
                      <Eye className="w-4 h-4" />
                    </button>
                    <button onClick={() => setViewerFile(file)}
                      className="w-9 h-9 rounded-xl bg-white/10 text-white flex items-center justify-center hover:scale-110 transition-transform" title="Fullscreen">
                ...
                {visibleFolders.map((folder) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-right">
                      <button onClick={(e) => openCtxMenu(e, "folder", folder.id)}
                        className="p-1 rounded hover:bg-white/10 text-white/20 hover:text-white transition-colors opacity-0 group-hover:opacity-100">
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100">
                        <button onClick={() => setViewerFile(file)}
                          className="p-1.5 rounded hover:bg-[#00b7ff]/20 text-white/30 hover:text-[#00b7ff] transition-colors" title="Open">
                          <Eye className="w-3.5 h-3.5" />
                        </button>
                        <button onClick={(e) => openCtxMenu(e, "file", file.id)}
                          className="p-1 rounded hover:bg-white/10 text-white/20 hover:text-white transition-colors">
                          <MoreVertical className="w-3.5 h-3.5" />
                        </button>
                      </div>
                    </td>
