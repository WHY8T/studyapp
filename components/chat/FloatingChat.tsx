...
import {
    ...
    Check, CheckCheck, Mic, MicOff, ImagePlus, Play, Pause,
    StopCircle,
} from "lucide-react";
    ...
    return (
        <div className="flex items-center gap-2 min-w-[160px]">
            ...
            <div className="flex-1 space-y-1">
                <div className={`h-1 rounded-full overflow-hidden ${isMine ? "bg-[#0D0D18]/20" : "bg-muted"}`}>
...
export default function FloatingChat() {
        ...
        const channel = supabase.channel(`chat:${currentUser.id}`)
            .on("postgres_changes", {
                ...
                if (selectedFriend?.id === msg.sender_id) {
                    ...
                    playNotifSound();
                    const senderName = friends.find((f) => f.id === msg.sender_id)?.username ?? "Friend";
                    toast({
                        title: `💬 ${senderName}`,
    ...
    useEffect(() => {
        ...
        const channel = supabase.channel(`study:${currentUser.id}`)
            .on("postgres_changes", {
                ...
                else if (room.status === "finished") { setStudyRoom(null); setTimeLeft(0); if (timerRef.current) clearInterval(timerRef.current); }
            }).subscribe();
        return () => { supabase.removeChannel(channel); };
    ...
    const startTimer = (room: StudyRoom) => {
        ...
        const tick = () => {
            ...
            if (remaining <= 0) { if (timerRef.current) clearInterval(timerRef.current); supabase.from("study_rooms").update({ status: "finished" }).eq("id", room.id); }
        };
    ...
    const sendMessage = async () => {
        ...
        const optimistic: Message = {
            id: Math.random().toString(), sender_id: currentUser.id,
            receiver_id: selectedFriend.id, content, type: "text",
    ...
    const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (uploadError) { toast({ title: "Upload failed", variant: "destructive" }); setUploadingImage(false); return; }

        const { data: { publicUrl } } = supabase.storage.from("chat-media").getPublicUrl(path);
        await supabase.from("messages").insert({
            sender_id: currentUser.id, receiver_id: selectedFriend.id,
            content: "", type: "image", media_url: publicUrl,
        });

        const newMsg: Message = {
            id: Math.random().toString(), sender_id: currentUser.id,
            receiver_id: selectedFriend.id, content: "", type: "image",
            media_url: publicUrl, read: false, created_at: new Date().toISOString(),
        };
    ...
    const stopRecording = async () => {
        ...
        if (duration < 1) return; // ignore too-short recordings
        ...
        await supabase.from("messages").insert({
            sender_id: currentUser.id, receiver_id: selectedFriend.id,
            content: "", type: "voice", media_url: publicUrl, duration_seconds: duration,
        });

        const newMsg: Message = {
            id: Math.random().toString(), sender_id: currentUser.id,
            receiver_id: selectedFriend.id, content: "", type: "voice",
            media_url: publicUrl, duration_seconds: duration,
            read: false, created_at: new Date().toISOString(),
        };
        setMessages((prev) => [...prev, newMsg]);
                        ...
                        {selectedFriend ? (
                            <>
                                ...
                                <div className="flex items-center gap-3 px-4 py-3 border-b border-border bg-card/95 backdrop-blur-sm shrink-0">
                                    ...
                                    <button onClick={sendStudyInvite} disabled={sendingStudyInvite || !!studyRoom}
                                        className="p-1.5 rounded-xl hover:bg-muted text-muted-foreground hover:text-lime transition-colors disabled:opacity-40" title="Study together">
                                    ...
                                    {recording ? (
                                        ...
                                        <div className="flex items-center gap-2 h-10">
                                            <div className="flex items-center gap-2 flex-1 px-3 py-2 rounded-xl bg-red-500/10 border border-red-500/30">
                                                ...
                                                <span className="text-sm font-mono text-red-400">{formatTime(recordingSeconds)}</span>
                                                <span className="text-xs text-muted-foreground">Recording...</span>
                                            </div>
                                            <button onClick={cancelRecording} className="p-2 rounded-xl hover:bg-muted text-muted-foreground transition-colors" title="Cancel">
                                                <X className="w-4 h-4" />
                                            </button>
                                            <button onClick={stopRecording} disabled={uploadingVoice}
                                                className="p-2 rounded-xl bg-red-500 text-white hover:bg-red-600 transition-colors" title="Send">
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <button onClick={() => imageInputRef.current?.click()} disabled={uploadingImage}
                                                className="p-2 rounded-xl hover:bg-muted text-muted-foreground hover:text-foreground transition-colors shrink-0" title="Send image">
                                            ...
                                            {newMessage.trim() ? (
                                                ...
                                                <button onClick={startRecording}
                                                    className="w-10 h-10 rounded-xl bg-lime/10 border border-lime/30 flex items-center justify-center text-lime hover:bg-lime/20 transition-colors shrink-0" title="Record voice">
